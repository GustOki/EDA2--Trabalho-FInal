...
int main(void) {
    ...
    CHECK(grafo_tem_aresta(g, 1, 0) == 1, "tem_aresta(1,0) == 1 (nao-direcionado)");
    CHECK(grafo_tem_aresta(g, 0, 2) == 0, "tem_aresta(0,2) == 0 (nao existe)");

    /* --- peso_aresta --- */
    CHECK(grafo_peso_aresta(g, 0, 1) == 2.5, "peso_aresta(0,1) correto");
    CHECK(grafo_peso_aresta(g, 1, 0) == 2.5, "peso_aresta(1,0) correto (simetrico)");
    ...
    CHECK(grafo_peso_aresta(g, 0, 1) == 2.5, "peso original preservado em duplicata");
